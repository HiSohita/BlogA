...
def blog(request):
    allpost=Post.objects.all()
    paginator = Paginator(allpost, 2)  # Paginate the items with 10 items per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    total_pages = paginator.num_pages
...
#     response = requests.get('http://localhost:8000/api/contact')
#     if response.status_code == 200:
#         data = response.json()
#         context = {
#             'contacts': data['results'],
#         }
#         return render(request, 'home/contact_list.html', context,{'data':data})
#     else:
